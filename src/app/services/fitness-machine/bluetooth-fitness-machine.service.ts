...
// This class is driven by Fitness Machine Service 1.0 bluetooth contract
// See: https://www.bluetooth.com/specifications/specs/fitness-machine-service-1-0/
// Contract assumptions
// 1. When setting power level ERG mode is automatically enabled
// 2. When setting resitance level ERG mode is automatically disabled
...
export class BluetoothFitnessMachineService implements FitnessMachineService {
  ...
  async connect(): Promise<void> {
    try {
      ...
      // Step 2: Check if GATT is available
      if (!this.device.gatt) {
        console.error('GATT is not available on this device.');
        throw new Error('GATT not available');
      }

      // Step 3: Connect to the GATT server on the device.
      try {
      ...
      if (supportedResistanceLevelRangeValue) {
        ...
        console.log('Support resistance level range:', this.supportedResistanceLevelRange);
      }
      ...
      if (supportedPowerRangeValue) {
        ...
        console.log('Support power range:', this.supportedPowerRange);
      }
